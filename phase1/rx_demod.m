...
if maxVal > normalizedThreshold
    ...
    if startIdx > 0 && (startIdx + N - 1 <= size(rxBuffer, 1))
        ...
        %% Optional: Constellation after matched filter
        figure(2); clf;
        subplot(1,2,1);
        plot(mf_ant1(1:10:end), '.');
        axis equal; grid on;
        title('Matched-Filter Constellation (Ant 1)');
        
        subplot(1,2,2);
        plot(mf_ant2(1:10:end), '.r');
        axis equal; grid on;
        title('Matched-Filter Constellation (Ant 2)');
